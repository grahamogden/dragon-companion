...
<template>
  ...
  <header class="fixed lg:relative top-0 w-full bg-header-sky bg-bottom dark:bg-top bg-auto lg:bg-cover header-transition duration-500">
  ...
  <div class="fixed lg:sticky top-8 lg:top-0 flex flex-col w-full z-10">
      ...
      <div class="flex flex-row p-0 pl-20 pr-16 h-8 lg:h-12 justify-between items-center max-w-page mx-auto relative">
        <div class="fixed lg:absolute top-0 lg:top-auto lg:bottom-0 left-0 lg:h-96 z-10">
          <router-link to="/" class="top-2 left-2 w-16 h-16 p-1 lg:p-0 overflow-visible lg:sticky"
            ...
            <img class="logo w-full h-full inline-block" src="@/assets/images/dragon-companion-logo.svg" alt="Dragon Companion logo" />
          </router-link>
        </div>
        ...
        <nav class="absolute lg:relative right-0 lg:right-auto h-full flex flex-row gap-x-8 pl-2 pr-20 lg:pr-10 justify-between items-center bg-biscay-800">
          ...
          <button class="relative h-8 w-[7rem] bg-theme-toggle bg-[length:5.6rem] bg-no-repeat bg-clip-content rounded-full border border-spring-wood-50 p-1" :class="{'bg-[center_top_0.4rem]': themeSetting === 'light', 'bg-[center_top_-0.7rem]': themeSetting === 'dark', 'bg-[center_top_-1.8rem]': themeSetting === 'auto'}" @click="toggleDarkMode"></button>
          <router-link class="text-spring-wood-50 hidden lg:inline-block no-underline hover:underline" v-if="!userAuthStore.isLoggedIn"
            :to="{ name: 'user-register' }"
            @click="toggleNavMenu(false); toggleAccountMenu(false)">Register</router-link>
          <router-link class="text-spring-wood-50 hidden lg:inline-block no-underline hover:underline" v-if="!userAuthStore.isLoggedIn"
            :to="{ name: 'login' }" @click="toggleNavMenu(false); toggleAccountMenu(false)">Log In</router-link>
          <router-link class="text-spring-wood-50 hidden lg:inline-block no-underline hover:underline" v-if="userAuthStore.isLoggedIn"
            :to="{ name: 'user-account' }" @click="toggleNavMenu(false); toggleAccountMenu(false)">Account</router-link>
          <button class="text-spring-wood-50 hidden lg:inline-block no-underline hover:underline" v-if="userAuthStore.isLoggedIn"
    ...
    <div class="hidden lg:block fixed lg:relative bottom-0">
      <div
        class="flex flex-row w-full bg-woodsmoke-950 border-b border-woodsmoke-900">
        <nav
            v-if="userAuthStore.isLoggedIn && campaignStore.isCampaignSelected" class="gap-x-4 flex flex-row text-center pl-20 w-full max-w-page mx-auto">
        ...
        <div
            v-if="!(userAuthStore.isLoggedIn && campaignStore.isCampaignSelected)" class="w-full max-w-page text-timberwolf-100 pl-20 pr-4 py-2 mx-auto">Please select a campaign above to start crafting your world and story!</div>
      </div>
    </div>
  </div>

  <main id="main-content" class="px-2 pt-16 pb-24 lg:py-2 h-full max-w-page mx-auto">
    <RouterView />
  </main>

  <nav class="fixed lg:hidden bottom-0 w-full toolbar grid gap-x-4 grid-cols-5 bg-woodsmoke-950 border-t border-woodsmoke-900">
    <router-link class="text-biscay-200 p-2"
      :to="{ name: 'characters', params: { externalCampaignId: campaignStore.campaignId } }"><img src="@/assets/images/dice-icon.svg" class="w-12 h-12 block border border-timberwolf-100 rounded" />Characters</router-link>
    <button type="button" class="text-biscay-200 p-2 underline" @click="toggleNavMenu(); toggleAccountMenu(false)"><img src="@/assets/images/dice-icon.svg" class="w-12 h-12 block border border-timberwolf-100 rounded" />More</button>
  </nav>
</template>