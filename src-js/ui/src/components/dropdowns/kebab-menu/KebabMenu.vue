...
<template>
    <div class="kebab-menu-container relative" v-on-click-outside="closeDropDownMenu">
        <div
            class="inline-block bg-timberwolf-50 dark:bg-woodsmoke-950 border border-white-lilac-800 dark:border-white-lilac-200 rounded-xl overflow-hidden transition-colors duration-500">
            <button @click="toggleKebabMenu" :aria-expanded="isMenuOpen"
                :aria-label="props.buttonAriaContextName + ' menu toggle'"
                class="px-2 py-1 text-white-lilac-800 hover:text-timberwolf-50 dark:text-white-lilac-200 dark:hover:text-woodsmoke-950 hover:bg-white-lilac-800 dark:hover:bg-white-lilac-200 no-underline transition-colors duration-0"
                type="button">
        ...
        <div v-show="isMenuOpen"
            ...
            <div v-for="link in props.links">
                <router-link v-if="link instanceof DropDownItemRouter" :to="link.destination!"
                    class="py-4 lg:py-2 px-6 block w-full hover:bg-woodsmoke-950/20 focus:bg-woodsmoke-950/20"
                ...
                <button v-if="link instanceof DropDownItemButton"
                    ...
                    class="py-4 lg:py-2 px-6 block w-full text-white-lilac-800 dark:text-white-lilac-200 border-0 hover:bg-woodsmoke-950/20 focus:bg-woodsmoke-950/20 underline hover:no-underline"
